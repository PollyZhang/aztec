...
public class IndexController extends WebController{
	...
	public void index(){
		try {
			List<Record> renmeixinwenList = Db.find("SELECT c.title,c.id,c.imageFile FROM tinyshop.t_contents as c"
					+ " where c.contentCategoryId = '509ffc01-2b79-4bfe-bc5a-675ebfe05208'");
			this.setAttr("renmeixinwenList", renmeixinwenList);
			List<Record> taokefengcaiList = Db.find("SELECT c.title,c.id,c.imageFile FROM tinyshop.t_contents as c"
					+ " where c.contentCategoryId = '7c24be68-0929-4b60-9f84-fab4ef6bc0c0'");
			this.setAttr("taokefengcaiList", taokefengcaiList);
			List<Record> qiyefengcaiList = Db.find("SELECT c.title,c.id,c.imageFile FROM tinyshop.t_contents as c"
					+ " where c.contentCategoryId = '39b1ccc3-e7b8-4042-8fda-148538fb7b57'");
			this.setAttr("qiyefengcaiList", qiyefengcaiList);
			List<Record> imageNewsList = Db.find("SELECT c.title,c.id,c.imageFile FROM tinyshop.t_contents as c"
